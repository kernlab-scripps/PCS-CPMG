...
Starting_Structure = "4QBH_Modeled_Hydrogens.pdb"
...
for M, M_File in zip(Metals, PCS_Files):
    with open(M_File, 'rb') as fh: 
        for line in fh.readlines(): 
            ...
            Prob_Dic[M][s[0]] = np.array([0.25, 0.25, 0.25, 0.25])
...
def Calc_Global_Sigma(Square_Dic, Prob_Dic):
    ...
    for M in Square_Dic:
        ...
        Sigma_Dic[M] = math.sqrt(np.sum(Sigma)/(len(Sigma)))
    return Sigma_Dic

def Update_Probabilities(Square_Dic, Sigma, Prob_Dic):
    # Calculate Prior 
    ...
    for M in Prob_Dic:
        ...
        aJ = np.sum(np.array(Prob_Array), axis=0)+1
        a0 = N + 4
        Prior[M] = np.exp(digamma(aJ)-digamma(a0))
    ...
    for M in Square_Dic:
        ...
        for Res in Square_Dic[M]:
            Gauss = s1*np.exp(s2*np.array(Square_Dic[M][Res]))*Prior[M]
            New_Prob_Dic[M][Res] = Gauss/np.sum(Gauss)+1e-9
    return New_Prob_Dic
...
def Calc_Total_Like(Square_Dic, Sigma, Prob_Dic):
    ...
    for M in Square_Dic:
        ...
        s3 = (-0.5)*(1.0/(Sigma[M]**2))
        for Res in Square_Dic[M]:
            Squared_Diff = np.array(Square_Dic[M][Res])
            LL = Prob_Dic[M][Res]*(s1+s2+s3*Squared_Diff +
                                  np.log(Prior[M]))
            LogLikes.append(np.sum(LL))
...
def ReCalculate_Scales(potList, PCS_Dic, Prob_Dic, Iteration,
    ...
    while abs(Likelihoods[-1]-Likelihoods[-2]) > 0.001:
        ...
        Prob_Dic = Update_Probabilities(Square_Dic, Sigma, Prob_Dic)
    ...
    Scale_Range = np.logspace(np.log10(0.001), np.log10(1), 1720)
    Scale_Range = np.concatenate((Scale_Range, np.array([1]*500)))
